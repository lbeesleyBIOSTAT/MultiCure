...
#' @param TransnewCov a list with elements: Trans13, Trans24, Trans14, Trans34, PNonCure. Each list element is a vector containing the names of the variables in newCov to be used in the model for the corresponding transition. 13 is NonCured -> Recurrence, 24 is Cured -> Death, 14 is NonCured -> Death, 34 is Recurrence -> Death. PNonCure contains the names of the newCovariates for the logistic regression for P(NonCure). 
...
STATEOCCUPANCYWEIB <- function(times, TransnewCov, newnewCov, beta, alpha, scale, shape) {
		  ...
		  shiny::sidebarLayout(
			shiny::sidebarPanel(
			 shiny::selectInput("plottype", "Plot Type:",
				...
				#sliderInput('CurTime', "Patient known to be alive at time: ", 0, min = 0, max = max(times)) ,
				#checkboxInput("RecurEvent", label = "Patient had observed recurrence", value = FALSE) ,
				#numericInput('RecurTime', "If Yes, when was the recurrence?: ", 0, min = 0, max = max(times))   ,
	...
	server = function(input, output ) {
		  ...
		  plotInput <- shiny::reactive({
			...
			if(length(beta) > length(c(TransnewCov$Trans14, TransnewCov$Trans24, TransnewCov$Trans13, TransnewCov$Trans34)) & !('INT' %in% TransnewCov$Trans14)){
		 	 	newCov$INT = rep(1,length(newCov[,1]))
				TransnewCov$Trans14 = c(TransnewCov$Trans14, 'INT')
			}
			...
			for(i in 1:length(newCov[,1])){	
				 local({
					...
					output[[plotname]]   <- renderPlot({
						...
						XB_alpha = as.numeric(alpha %*% t(cbind(1, newCovTEMP[,TransnewCov$PNonCure])))
						prob_Noncure = exp(XB_alpha)/(1+exp(XB_alpha))
						
						A1 = length(TransnewCov$Trans13)
						A2 = length(TransnewCov$Trans24)
						A3 = length(TransnewCov$Trans14)
						A4 = length(TransnewCov$Trans34)
						TRANS = c(rep(1,A1), rep(2,A2), rep(3,A3), rep(4,A4))
						XB_beta13 = as.numeric(beta[TRANS==1] %*% t(cbind(newCovTEMP[,TransnewCov$Trans13])))	
						XB_beta24 = as.numeric(beta[TRANS==2] %*% t(cbind(newCovTEMP[,TransnewCov$Trans24])))	
						XB_beta14 = as.numeric(beta[TRANS==3] %*% t(cbind(newCovTEMP[,TransnewCov$Trans14])))	
						XB_beta34 = as.numeric(beta[TRANS==4] %*% t(cbind(newCovTEMP[,TransnewCov$Trans34])))		
						...
						if(input$CurTime == 0 & input$RecurEvent==FALSE){
							...
							# plot(Prob2Save, ylim = c(0,1))
							# lines(Prob1Save, col = 'red')
							# lines(Prob3Save, col = 'blue')
							# lines(Prob4Save, col = 'green')
